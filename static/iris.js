...
function convertToTensors(data, targets, testSplit) {
  ...
  // dividing all data by var float
...
function getIrisData(testSplit) {
  return tf.tidy(() => {
    ...
    for (let i = 0; i < IRIS_CLASSES.length; i += 1) {
      dataByClass.push([]);
      targetsByClass.push([]);
    }
    for (const example of IRIS_DATA) {
      ...
      // taking class out of data
      const data = example.slice(0, example.length - 1);
      //creating two array of arrays that correspond by class
    ...
    // show results
    const concatAxis = 0;
    tf.concat(xTrains, concatAxis).print()
    tf.concat(yTrains, concatAxis).print()
    tf.concat(xTests, concatAxis).print()
    tf.concat(yTests, concatAxis).print()
